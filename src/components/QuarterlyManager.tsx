...
import { getQuarterRange, QUARTERLY_SECTION_OPTIONS } from '../shared/quarterly';
import type {
  AiSettingsState,
...
interface QuarterlyManagerProps {
...
const shiftQuarter = (value: string, amount: number) => {
  const [yearText, quarterText] = value.split('-Q');
  const date = new Date(Number(yearText), (Number(quarterText) - 1 + amount) * 3, 1);
  return `${date.getFullYear()}-Q${Math.floor(date.getMonth() / 3) + 1}`;
};

const errorMessage = (error: unknown) => error instanceof Error
  ? error.message.replace(/^Error invoking remote method '[^']+': Error: /, '')
  : '분기 정리를 처리하지 못했습니다. 원본 기록은 그대로 유지됩니다.';
...
export default function QuarterlyManager({
  aiState,
  onOpenAiSettings,
  onOpenRecord,
  onOpenDocument,
}: QuarterlyManagerProps) {
  const [quarterKey, setQuarterKey] = useState(() => getQuarterRange().quarterKey);
  const [workspace, setWorkspace] = useState<QuarterlyWorkspace | null>(null);
  const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const [reflectionLevel, setReflectionLevel] = useState(50);
  const [draft, setDraft] = useState<QuarterlySummaryDraft | null>(null);
  ...
  const loadWorkspace = useCallback(async (nextQuarter: string, keepNotice = false) => {
    try {
      const next = await window.itta.getQuarterlyWorkspace({ quarterKey: nextQuarter });
      setWorkspace(next);
  ...
  useEffect(() => { void loadWorkspace(quarterKey); }, [quarterKey, loadWorkspace]);
  ...
  const toggleRecord = (recordId: string) => setSelectedIds((current) => {
    ...
    if (current.length >= 200) {
      setNotice('분기 정리에는 기록을 최대 200개까지 선택할 수 있습니다.');
      return current;
    }
  ...
  const generate = async () => {
    if (!workspace || selectedIds.length === 0) {
      setNotice('분기 정리에 사용할 기록을 한 개 이상 선택해 주세요.');
      return;
    }
    if (!aiState.session.connected) {
      setNotice('분기 정리를 만들려면 메뉴에서 AI를 다시 연결해 주세요.');
      return;
    }
    setIsWorking(true);
    setNotice(null);
    try {
      setDraft(await window.itta.generateQuarterlySummary({
        quarterKey: workspace.quarterKey,
        quarterStart: workspace.quarterStart,
        quarterEnd: workspace.quarterEnd,
  ...
  const updateResult = <Key extends keyof QuarterlySummaryResult>(
    key: Key,
    value: QuarterlySummaryResult[Key],
  ...
  const saveDraft = async () => {
    ...
    setNotice(null);
    try {
      await window.itta.saveQuarterlySummary({ draft });
      setNotice('검토·채택한 분기 교무수첩을 Markdown으로 저장했습니다.');
      await loadWorkspace(draft.quarterKey, true);
      setActiveTab('saved');
    } catch (error) {
      setNotice(errorMessage(error));
    } finally {
  ...
  if (!workspace) {
    return <section className="weekly-manager weekly-loading"><p>분기 기록을 불러오고 있습니다…</p></section>;
  }

  const [selectedYear, selectedQuarter] = quarterKey.split('-Q');
  const currentYear = new Date().getFullYear();
  const yearOptions = Array.from({ length: 21 }, (_, index) => currentYear - 10 + index);
  const allSelectableIds = visibleRecords.slice(0, 200).map((record) => record.id);
  const allSelected = allSelectableIds.length > 0
    && allSelectableIds.every((recordId) => selectedIds.includes(recordId));

  return (
    <section className="weekly-manager" aria-labelledby="quarterly-title">
      <div className="weekly-heading">
        <div>
          <p className="eyebrow">세 달의 연결과 변화를 살펴보는</p>
          <h1 id="quarterly-title">분기 교무수첩</h1>
          <p>{formatQuarter(workspace.quarterKey)} · {formatDate(workspace.quarterStart)}부터 {formatDate(workspace.quarterEnd)}까지</p>
          <div className="data-use-context"><DataUseBadge scope="ai" detail="교사가 선택하고 확인한 분기 비식별 기록만 AI API로 전송됩니다." /></div>
        </div>
        <div className="week-navigation">
          <button type="button" onClick={() => setQuarterKey((current) => shiftQuarter(current, -1))}>← 이전 분기</button>
          <label>
            <span className="sr-only">연도 선택</span>
            <select value={selectedYear} onChange={(event) => setQuarterKey(`${event.target.value}-Q${selectedQuarter}`)}>
              {yearOptions.map((year) => <option key={year} value={year}>{year}년</option>)}
            </select>
          </label>
          <label>
            <span className="sr-only">분기 선택</span>
            <select value={selectedQuarter} onChange={(event) => setQuarterKey(`${selectedYear}-Q${event.target.value}`)}>
              {[1, 2, 3, 4].map((quarter) => <option key={quarter} value={quarter}>{quarter}분기</option>)}
            </select>
          </label>
          <button type="button" onClick={() => setQuarterKey((current) => shiftQuarter(current, 1))}>다음 분기 →</button>
        </div>
      </div>
      ...
      {!draft && (
        <div className="period-tabs" role="tablist" aria-label="분기 교무수첩 보기 선택">
      ...
      {!draft && activeTab === 'saved' && (
        workspace.summaries.length > 0 ? (
          <section className="saved-weekly-list">
            <div><h2>저장한 분기 교무수첩</h2><span>{workspace.summaries.length}개</span></div>
            {workspace.summaries.map((summary) => (
              <article key={summary.id}><span><strong>{summary.result.overview || '분기 교무수첩'}</strong><small>{summary.provider} · 근거 기록 {summary.recordIds.length}개</small></span><button type="button" onClick={() => onOpenDocument(summary.id)}>문서 보기</button></article>
            ))}
          </section>
        ) : (
          <div className="period-empty-state"><strong>아직 저장한 분기 요약이 없습니다.</strong><p>세 달의 기록으로 첫 요약을 만들어 보세요.</p><button className="primary-button" type="button" onClick={() => setActiveTab('new')}>새 요약 만들기</button></div>
        )
      )}
        ...
        <>
          <div className="weekly-selection-card">
            <div className="weekly-selection-heading">
              <div>
                <h2>이번 분기 근거 기록 선택</h2>
                <p>선택한 비식별 기록만 AI에 전송됩니다. 최대 200개까지 선택할 수 있습니다.</p>
              </div>
              <label>
            ...
            {visibleRecords.length === 0 ? (
              <div className="weekly-empty"><strong>이 분기에는 저장된 기록이 없습니다.</strong><p>다른 분기를 선택하거나 먼저 교실 기록을 작성해 주세요.</p></div>
            ) : (
            ...
            <div className="weekly-generate-actions">
              ...
              <button className="primary-button" type="button" disabled={isWorking || selectedIds.length === 0 || selectedCharacters > 50_000} onClick={generate}>
                {isWorking ? '분기의 흐름을 정리하는 중…' : '선택 기록으로 AI 분기 정리'}
              </button>
            </div>
          </div>
      ...
      {draft && (
        <section className="weekly-draft-review">
          <div className="weekly-draft-heading">
            <div><p className="eyebrow">AI 초안 · 교사가 최종 결정</p><h2>분기 정리를 검토하고 고쳐 주세요</h2></div>
          ...
          <div className="weekly-result-fields">
            <label><span>이번 분기 주요 흐름</span><textarea value={draft.result.overview} onChange={(event) => updateResult('overview', event.target.value)} /></label>
            {QUARTERLY_SECTION_OPTIONS.filter((section) => section.id !== 'overview').map((section) => {
              const key = section.id as Exclude<keyof QuarterlySummaryResult, 'overview'>;
