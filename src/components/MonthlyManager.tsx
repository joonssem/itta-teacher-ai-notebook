...
import { MONTHLY_SECTION_OPTIONS } from '../shared/monthly';
import type {
  AiSettingsState,
  MonthlySummaryDraft,
...
const errorMessage = (error: unknown) => {
  ...
  return '월간 정리를 처리하지 못했습니다. 원본 기록은 그대로 유지됩니다.';
};
...
export default function MonthlyManager({
  ...
  const [draft, setDraft] = useState<MonthlySummaryDraft | null>(null);
  ...
  const loadWorkspace = useCallback(async (nextMonth: string, keepNotice = false) => {
    try {
      const next = await window.itta.getMonthlyWorkspace({ monthKey: nextMonth });
      setWorkspace(next);
      setSelectedIds(next.records.slice(0, 100).map((record) => record.id));
      setDraft(null);
      if (!keepNotice) {
        setNotice(null);
      }
  ...
  useEffect(() => {
    void loadWorkspace(monthKey);
  }, [monthKey, loadWorkspace]);
  ...
  const toggleRecord = (recordId: string) => setSelectedIds((current) => {
    ...
    if (current.length >= 100) {
      setNotice('월간 정리에는 기록을 최대 100개까지 선택할 수 있습니다.');
      return current;
    }
  ...
  const generate = async () => {
    if (!workspace || selectedIds.length === 0) {
      setNotice('월간 정리에 사용할 기록을 한 개 이상 선택해 주세요.');
      return;
    }
    if (!aiState.session.connected) {
      setNotice('월간 정리를 만들려면 메뉴에서 AI를 다시 연결해 주세요.');
      return;
    }
    setIsWorking(true);
    setNotice(null);
    try {
      setDraft(await window.itta.generateMonthlySummary({
  ...
  const updateResult = <Key extends keyof MonthlySummaryResult>(
    key: Key,
    value: MonthlySummaryResult[Key],
  ...
  const saveDraft = async () => {
    ...
    setNotice(null);
    try {
      await window.itta.saveMonthlySummary({ draft });
      setNotice('검토·채택한 월간 교무수첩을 Markdown으로 저장했습니다.');
      await loadWorkspace(draft.monthKey, true);
      setActiveTab('saved');
    } catch (error) {
      setNotice(errorMessage(error));
    } finally {
  ...
  if (!workspace) {
    return <section className="weekly-manager weekly-loading"><p>월간 기록을 불러오고 있습니다…</p></section>;
  }

  const allSelectableIds = visibleRecords.slice(0, 100).map((record) => record.id);
  const allSelected = allSelectableIds.length > 0
    && allSelectableIds.every((recordId) => selectedIds.includes(recordId));

  return (
    <section className="weekly-manager" aria-labelledby="monthly-title">
      <div className="weekly-heading">
        <div>
          <p className="eyebrow">한 달의 기록에서 흐름을 발견하는</p>
          <h1 id="monthly-title">월간 교무수첩</h1>
          <p>{formatMonth(workspace.monthKey)} · {formatDate(workspace.monthStart)}부터 {formatDate(workspace.monthEnd)}까지</p>
          <div className="data-use-context"><DataUseBadge scope="ai" detail="교사가 선택하고 확인한 월간 비식별 기록만 AI API로 전송됩니다." /></div>
        </div>
        <div className="week-navigation">
          <button type="button" onClick={() => setMonthKey((current) => shiftMonth(current, -1))}>← 이전 달</button>
          <label>
            <span className="sr-only">정리할 달 선택</span>
            <input type="month" value={monthKey} onChange={(event) => setMonthKey(event.target.value)} />
          </label>
          <button type="button" onClick={() => setMonthKey((current) => shiftMonth(current, 1))}>다음 달 →</button>
        </div>
      </div>
      ...
      {!draft && (
        <div className="period-tabs" role="tablist" aria-label="월간 교무수첩 보기 선택">
      ...
      {!draft && activeTab === 'saved' && (
        workspace.summaries.length > 0 ? (
          <section className="saved-weekly-list">
            <div><h2>저장한 월간 교무수첩</h2><span>{workspace.summaries.length}개</span></div>
            {workspace.summaries.map((summary) => (
              <article key={summary.id}>
                <span><strong>{summary.result.overview || '월간 교무수첩'}</strong><small>{summary.provider} · 근거 기록 {summary.recordIds.length}개</small></span>
                <button type="button" onClick={() => onOpenDocument(summary.id)}>문서 보기</button>
              </article>
            ))}
          </section>
        ) : (
          <div className="period-empty-state"><strong>아직 저장한 월간 요약이 없습니다.</strong><p>이달 기록으로 첫 요약을 만들어 보세요.</p><button className="primary-button" type="button" onClick={() => setActiveTab('new')}>새 요약 만들기</button></div>
        )
      )}
        ...
        <>
          <div className="weekly-selection-card">
            <div className="weekly-selection-heading">
              <div>
                <h2>이번 달 근거 기록 선택</h2>
                <p>선택한 비식별 기록만 AI에 전송됩니다. 최대 100개까지 선택할 수 있습니다.</p>
              </div>
              <label>
                <input
                  type="checkbox"
            ...
            {visibleRecords.length === 0 ? (
              <div className="weekly-empty">
                <strong>{workspace.records.length === 0 ? '이 달에는 저장된 기록이 없습니다.' : '조건에 맞는 기록이 없습니다.'}</strong>
                <p>{workspace.records.length === 0 ? '다른 달을 선택하거나 먼저 교실 기록을 작성해 주세요.' : '검색어나 카테고리를 바꿔 보세요.'}</p>
              </div>
            ) : (
            ...
            <div className="weekly-generate-actions">
              ...
              <button
                ...
                onClick={generate}
              >
                {isWorking ? '한 달의 흐름을 정리하는 중…' : '선택 기록으로 AI 월간 정리'}
              </button>
            </div>
          </div>
      ...
      {draft && (
        <section className="weekly-draft-review">
          ...
          <div className="weekly-result-fields">
            <label>
              <span>이번 달 주요 흐름</span>
              <textarea value={draft.result.overview} onChange={(event) => updateResult('overview', event.target.value)} />
            </label>
            {MONTHLY_SECTION_OPTIONS.filter((section) => section.id !== 'overview').map((section) => {
              const key = section.id as Exclude<keyof MonthlySummaryResult, 'overview'>;
              return (
